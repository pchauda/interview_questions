package com.pc.interview.questions.cracking_the_coding_interview.trees_graphs;

/**
 * <p>Implement a function to check if a binary tree is balanced. For the purposes of this question, a balanced tree is
 * defined to be a tree such that the heights of the two subtrees of any node never differ by more than one.</p>
 */
public class IsBalancedBinaryTree {
    ...
    private Pair isBalanced(TreeNode root) {
        ...
        if (Math.abs(leftTree.height - rightTree.height) > 1)
            return new Pair(Math.max(leftTree.height, rightTree.height), false);
        else return new Pair(1 + Math.max(leftTree.height, rightTree.height), true);
    }

    // Using error code (max, min value of int) to identify the fact that subtree is not balanced.
